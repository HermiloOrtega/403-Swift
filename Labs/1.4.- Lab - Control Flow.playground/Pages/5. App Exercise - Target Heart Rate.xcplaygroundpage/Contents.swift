...
let currentHR = 147
let isInTarget: Bool = currentHR <= targetUpperBound && currentHR >= targetLowerBound ? true : false
let isBelowTarget: Bool = currentHR < targetLowerBound ? true : false
let isAboveTarget: Bool = currentHR > targetUpperBound ? true : false

print(isInTarget ? "You're right on track!" : isBelowTarget ? "You're doing great, but try to push it a bit!" : "You're on fire! Slow it down just a bit")
